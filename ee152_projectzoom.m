...
function enhanced_image = processSatelliteImage(img)
    ...
    zoom_frac = 0.25; % 1/4 width and height => 1/16 of image area

    % Show Original Color Image
    subplot(2,3,1), imshow(cropCenterRegion(img, zoom_frac)), title('Original Image', 'FontWeight','bold');
    ...
    subplot(2,3,2), imshow(cropCenterRegion(magnitude_spectrum, zoom_frac), []), title('FFT Spectrum', 'FontWeight','bold');
    ...
    subplot(2,3,3), imshow(cropCenterRegion(H_butter, zoom_frac), []), title('Butterworth Filter Mask', 'FontWeight','bold');
    ...
    subplot(2,3,5), imshow(cropCenterRegion(img_gauss_color, zoom_frac)), title('Gaussian High-Pass', 'FontWeight','bold');

    % Histogram Equalization
    img_hist_eq = histeq(img_gray);
    img_hist_eq_color = applyGrayToColor(img, img_hist_eq);
    subplot(2,3,6), imshow(cropCenterRegion(img_hist_eq_color, zoom_frac)), title('Histogram Equalized', 'FontWeight','bold');
...
%% Crop Center Region Function (Zoom to inner 1/8th)
function cropped = cropCenterRegion(img, frac)
    [M, N, C] = size(img);
    h = round(M * frac);
    w = round(N * frac);
    y1 = round((M - h)/2);
    x1 = round((N - w)/2);
    cropped = imcrop(img, [x1, y1, w-1, h-1]);
    % Ensure 3 channels if grayscale image
    if C == 1 && ndims(cropped) < 3
        cropped = repmat(cropped, [1, 1, 3]);
    end
end